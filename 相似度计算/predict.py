...
df = pd.read_excel('train_pair_sim.xlsx')
df = df[~df['sentence1'].isnull()]
texts_list = df['sentence1'].values.tolist()
texts = []
for i in texts_list:
    text_str = str(i)
    if len(i) > 0:
...
tokenizer = Tokenizer(r"/opt/modules/vec/vec-vocab.txt", word_maxlen=max_seg_len,
                      do_lower_case=True)
# tokenizer = Tokenizer(r"D:\models\shanghai\vec-vocab.txt", word_maxlen=max_seg_len,
#                       do_lower_case=True)
...
# 本地
# sess = onnxruntime.InferenceSession(r"D:\models\shanghai\vec.onnx",
...
#                                     providers=['CPUExecutionProvider'])
# gpu
sess = onnxruntime.InferenceSession(r"/opt/modules/vec/vec.onnx",
                                    providers=['CUDAExecutionProvider'])
...
with open("train_pair_sim.npy", "wb") as file_cache:
    joblib.dump({"keys": texts, "vecs": re_all_vec}, file_cache)
print(len(texts))
print(len(re_all_vec))
np.save('train_pair_sim_npy.npy',re_all_vec)
